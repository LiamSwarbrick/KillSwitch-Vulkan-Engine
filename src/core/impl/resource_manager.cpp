...
unload_record(ResourceManager* manager, ResourceRecord* record)
{
    ...
    if (record->payload_kind == RESOURCE_PAYLOAD_BINARY_BYTES)
    {
        if (record->binary_data != NULL)
        {
            L_free(record->binary_data, &manager->tt);
            record->binary_data = NULL;
        }
    }
    else if (record->payload_kind == RESOURCE_PAYLOAD_GLTF_ASSET)
    {
        if (record->asset_data != NULL)
        {
            free_asset(record->asset_data);
            record->asset_data = NULL;
        }
    }

    record->byte_size = 0;
    record->payload_kind = RESOURCE_PAYLOAD_NONE;
...
load_record_now(ResourceManager* manager, ResourceRecord* record)
{
    ...
    if (record->state == RESOURCE_LOAD_STATE_LOADED)
    {
        if (record->payload_kind == RESOURCE_PAYLOAD_BINARY_BYTES && record->binary_data != NULL) return 1;
        if (record->payload_kind == RESOURCE_PAYLOAD_GLTF_ASSET && record->asset_data != NULL) return 1;
    }
    ...
    if (record->type == RESOURCE_TYPE_GLTF_ASSET)
    {
        Asset* asset = load_asset(record->source_path);
        if (asset == NULL)
        {
            record->state = RESOURCE_LOAD_STATE_FAILED;
            SDL_LogError(
                SDL_LOG_CATEGORY_APPLICATION,
                "ResourceManager: failed to load glTF asset '%s' from '%s'",
                record->logical_name,
                record->source_path
            );
            return 0;
        ...
        record->payload_kind = RESOURCE_PAYLOAD_GLTF_ASSET;
        record->asset_data = asset;
        record->binary_data = NULL;
        record->byte_size = 0;
        ...
        SDL_Log(
            "ResourceManager: loaded glTF asset [%u] '%s'",
            record->handle,
            record->logical_name
        );
    ...
    else
    {
        ...
        if (bytes == NULL)
        {
            ...
            SDL_LogError(
                SDL_LOG_CATEGORY_APPLICATION,
                "ResourceManager: failed to load binary resource '%s' from '%s'",
                record->logical_name,
                record->source_path
            );
            return 0;
        ...
        record->payload_kind = RESOURCE_PAYLOAD_BINARY_BYTES;
        record->binary_data = bytes;
        record->asset_data = NULL;
        ...
        SDL_Log(
            "ResourceManager: loaded binary resource [%u] '%s' (%" PRIu64 " bytes)",
            record->handle,
            record->logical_name,
            record->byte_size
        );
...
{
    return ResourceManager_RequestFile(manager, type, residency, logical_name, source_path);
}

ResourceHandle
ResourceManager_RequestGLTFAsset(
    ResourceManager* manager,
    ResourceResidency residency,
    const char* logical_name,
    const char* source_path
...
{
    return ResourceManager_RequestFile(
        manager,
...
const void*
ResourceManager_GetBinaryData(const ResourceManager* manager, ResourceHandle handle)
{
    const ResourceRecord* record = get_record_const(manager, handle);
    if (record == NULL || record->payload_kind != RESOURCE_PAYLOAD_BINARY_BYTES)
    {
        return NULL;
    }
...
u64
ResourceManager_GetBinarySize(const ResourceManager* manager, ResourceHandle handle)
{
    const ResourceRecord* record = get_record_const(manager, handle);
    if (record == NULL || record->payload_kind != RESOURCE_PAYLOAD_BINARY_BYTES)
    {
    ...
    return record->byte_size;
}

Asset*
ResourceManager_GetGLTFAsset(const ResourceManager* manager, ResourceHandle handle)
{
    const ResourceRecord* record = get_record_const(manager, handle);
    if (record == NULL || record->payload_kind != RESOURCE_PAYLOAD_GLTF_ASSET)
    {
        return NULL;
    }

    return record->asset_data;
}
...
ResourceManager_LogSummary(const ResourceManager* manager)
{
    ...
    for (u32 i = 0; i < manager->record_count; ++i)
    {
        const ResourceRecord* record = &manager->records[i];
        SDL_Log(
            "[%u] name='%s' type=%d residency=%d state=%d refs=%u gen=%u payload=%d bytes=%" PRIu64 " backend_rid=%u path='%s'",
            record->handle,
            record->logical_name,
            (int)record->type,
