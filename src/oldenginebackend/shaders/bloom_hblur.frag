...
layout (set = 1, binding = 0) uniform sampler2D brightness_texture;
...
void main()
{
    float texel_width = 1.0 / textureSize(brightness_texture, 0).x;

    // Center tap
    vec3 color = texture(brightness_texture, v2f_uv).rgb * CENTER_WEIGHT;

    // Paired taps
    for (int i = 0; i < PAIRED_WEIGHT_COUNT; ++i)
    {
        vec2 offset = vec2(offsets[i] * texel_width, 0.0);

        vec3 sample_pos_side = texture(brightness_texture, v2f_uv + offset).rgb;
        vec3 sample_neg_side = texture(brightness_texture, v2f_uv - offset).rgb;
