...
void PlayerMovement_Update(ECS* ecs, float dt)
{
    ...
    view.ForEach([&](EntityID e,
        C_PlayerInput& input,
        C_CharacterController& controller,
        C_Transform& transform,
        C_AnimatedMesh& animatedMesh)
        {
            glm::vec3 moveDir(0.0f);

            if (input.move_forward)  moveDir.z -= 1.0f;
            if (input.move_backward) moveDir.z += 1.0f;
            if (input.move_left)     moveDir.x -= 1.0f;
            if (input.move_right)    moveDir.x += 1.0f;
