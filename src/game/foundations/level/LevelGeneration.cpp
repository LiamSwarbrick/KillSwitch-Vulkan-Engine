...
void LevelGeneration::BuildPalette(const std::vector<Asset*>& roomAssets)
{
	...
	for (Asset* asset : roomAssets)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (!roomDuplicate)
				palette.push_back({ asset, rotatedMask, rotation });
		}
	}
}

void LevelGeneration::GenerateGrid(int width, int height)
{
	gridWidth = width;
	gridHeight = height;
	...
	while (true)
	{
		...
		int gridIndex = nextY * width + nextX;
		GridCell& cell = grid[gridIndex];

		std::mt19937 rng(std::random_device{}());
		std::uniform_int_distribution<> dist(0, cell.validRooms.size() - 1);
		int roomChoice = cell.validRooms[dist(rng)];
		//int roomChoice = cell.validRooms[rand() % cell.validRooms.size()];
		...
		while (!updateRooms.empty())
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				if (neighbourRoom.x < 0 || neighbourRoom.x >= width || neighbourRoom.y < 0 || neighbourRoom.y >= height)
					continue;

				// Find neighbour gridcell and check if its already collapsed
				int currentGridIndex = currentRoom.y * width + currentRoom.x;
				GridCell& current = grid[currentGridIndex];
				int neighbourGridIndex = neighbourRoom.y * width + neighbourRoom.x;
